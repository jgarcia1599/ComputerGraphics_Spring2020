function objInit(Obj){
	...
	createBuffers();    /* buffers for positions, normals etc. */
	sendBufferData();   
...
/*----- Attach data and functions to Obj -----*/
	...
	Obj.updateBufferData = function(){
		/* Update data in buffers. We assume that the sizes of the 
		   buffers don't need to be changed */

		/* normals need to be recomputed even if initially passed
		   since changing positions of vertices may change normals */


		computeNormals(); 

		if(usingNormalMap) computeTangentsAndBitangents();
